...
1. 支援無 path 或單一檔案 path；指定檔案時只讀取 staged 內容，不替使用者 `git add`。
2. 未指定 path 時，處理 repository 中所有 tracked 的 staged 與 unstaged 變更，commit 語意等同 `git commit -a`，但不包含 untracked 檔案。
