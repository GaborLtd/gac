...
- provider 若能列 model，onboarding 顯示編號清單；每個項目分為 model ID 與人類可讀的 display name，設定檔保存 provider 可接受的 model value（agy 使用 model ID），使用者也可選擇手動輸入。
...
- onboarding 預設推薦多個低成本 model；使用者可選 model，也可在設定中永久覆寫。Codex 候選另保存 reasoning effort，預設使用 low；目前 Codex CLI 透過 `--config model_reasoning_effort="low"` 傳入，不使用不存在的 `--effort` flag。
